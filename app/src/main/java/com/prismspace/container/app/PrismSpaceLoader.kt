...
import com.prismspace.container.app.rocker.RockerManager
import com.prismspace.container.biz.cache.AppSharedPreferenceDelegate
...
class PrismSpaceLoader {
    ...
    fun getPrismSpaceCore(): PrismSpaceCore {
        return try {
            PrismSpaceCore.get()
        } catch (e: Exception) {
                            ...
                            object : AppLifecycleCallback() {
                                override fun beforeCreateApplication(
                                    ...
                                    try {
                                        Log.d(
                                                TAG,
                                                "beforeCreateApplication: pkg $packageName, processName $processName,userID:${PActivityThread.getUserId()}"
                                        )
                                    } catch (e: Exception) {
                                    ...
                                    try {
                                        ...
                                        val intent =
                                                android.content.Intent(
                                                        "com.prismspace.container.REQUEST_STORAGE_PERMISSION"
                                                )
    ...
    fun doOnCreate(context: Context) {
        engineScope.launch {
            try {
                ...
                try {
                    PrismSpaceCore.get().addServiceAvailableCallback {
    ...
    companion object {
        val TAG: String = PrismSpaceLoader::class.java.simpleName
    }
}
