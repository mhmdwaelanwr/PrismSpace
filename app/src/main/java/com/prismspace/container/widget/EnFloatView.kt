...
class EnFloatView(mContext: Context) : FloatingMagnetView(mContext) {
    ...
    private fun initRockerView() {
        ...
        rockerView?.setListener(object : RockerView.RockerListener {
            override fun callback(eventType: Int, currentAngle: Float, currentDistance: Float) {
                if (eventType == RockerView.EVENT_CLOCK && currentAngle != -1F) {
