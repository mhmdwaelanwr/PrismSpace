...
public class MethodParameterUtils {
    ...
    public static String replaceFirstAppPkg(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof String) {
                ...
                if (PrismSpaceCore.get().isInstalled(value, PrismSpaceCore.getUserId())) {
                    args[i] = PrismSpaceCore.getHostPkg();
                    return value;
                }
            }
        }
    ...
    public static void replaceFirstUid(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof Integer) {
                int uid = (int) args[i];
                if (uid == PrismSpaceCore.getBUid()) {
                    args[i] = PrismSpaceCore.getHostUid();
                }
            }
        }
    }
    ...
    public static String replaceLastAppPkg(Object[] args) {
        ...
        if (index != -1) {
            ...
            if (PrismSpaceCore.get().isInstalled(pkg, PrismSpaceCore.getUserId())) {
                args[index] = PrismSpaceCore.getHostPkg();
            }
            return pkg;
        }
        return null;
