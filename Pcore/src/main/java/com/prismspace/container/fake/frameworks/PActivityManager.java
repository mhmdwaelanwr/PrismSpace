...
import com.prismspace.container.core.system.am.IBActivityManagerService;
...
public class PActivityManager extends PlackManager<IBActivityManagerService> {
    private static final String TAG = "PActivityManager";
    ...
    public String getPackageForIntentSender(IBinder target) {
        try {
            return getService().getPackageForIntentSender(target, PActivityThread.getUserId());
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public int getUidForIntentSender(IBinder target) {
        try {
            return getService().getUidForIntentSender(target, PActivityThread.getUserId());
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return -1;
