...
import com.prismspace.container.core.system.pm.IBPackageManagerService;
...
import com.prismspace.container.utils.TransactionThrottler;


public class PPackageManager extends PlackManager<IBPackageManagerService> {
    ...
    public IBPackageManagerService getServiceWithFallback() {
        ...
        if (service == null) {
            ...
            try {
                PrismSpaceCore.get().ensureCorePackageServicesReady(2500L);
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        List<ResolveInfo> ris = queryIntentActivities(intentToResolve,
                0,
                intentToResolve.resolveTypeIfNeeded(PrismSpaceCore.getContext().getContentResolver()),
                userId);
    ...
    private Intent createFallbackLaunchIntent(String packageName) {
        try {
            ...
            Intent intent = PrismSpaceCore.getContext().getPackageManager().getLaunchIntentForPackage(packageName);
            if (intent != null) {
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                return intent;
            }
        ...
        try {
            ...
            if (file != null && !file.isEmpty()) {
                try {
                    ...
                    PackageInfo packageInfo = PrismSpaceCore.getPackageManager().getPackageArchiveInfo(file, 0);
                    if (packageInfo != null) {
                        String packageName = packageInfo.packageName;
                        String hostPackageName = PrismSpaceCore.getHostPkg();
                        if (packageName.equals(hostPackageName)) {
                            Log.w(TAG, "Attempt to install PrismSpace host app detected and blocked: " + packageName);
            ...
            IBPackageManagerService service = getServiceWithFallback();
            if (service == null) {
                Log.e(TAG, "PackageManager service unavailable during installPackageAsUser: " + file);
    ...
    public List<ApplicationInfo> getInstalledApplications(int flags, int userId) {
        try {
            IBPackageManagerService service = getServiceWithFallback();
            if (service == null) {
                Log.w(TAG, "PackageManager service unavailable in getInstalledApplications");
                return Collections.emptyList();
            }
            return service.getInstalledApplications(flags, userId);
        } catch (RemoteException e) {
            Log.e(TAG, "RemoteException in getInstalledApplications", e);
        }
        return Collections.emptyList();
    }
    ...
    public List<PackageInfo> getInstalledPackages(int flags, int userId) {
        try {
            IBPackageManagerService service = getServiceWithFallback();
            if (service == null) {
                Log.w(TAG, "PackageManager service unavailable in getInstalledPackages");
                return Collections.emptyList();
            }
            return service.getInstalledPackages(flags, userId);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return Collections.emptyList();
    ...
    public void stopPackage(String packageName, int userId) {
        try {
            IBPackageManagerService service = getServiceWithFallback();
            if (service != null) {
                service.stopPackage(packageName, userId);
            }
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    ...
    public void uninstallPackageAsUser(String packageName, int userId) {
        try {
            IBPackageManagerService service = getServiceWithFallback();
            if (service != null) {
                service.uninstallPackageAsUser(packageName, userId);
            }
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    ...
    private boolean isInstalledFallback(String packageName) {
        try {
            ...
            PrismSpaceCore.getContext().getPackageManager().getPackageInfo(packageName, 0);
            return true;
        } catch (Exception e) {
            ...
            if (packageName != null && (packageName.equals("com.media.bestrecorder.audiorecorder") || 
                                       packageName.startsWith("com.prismspace.container"))) {
