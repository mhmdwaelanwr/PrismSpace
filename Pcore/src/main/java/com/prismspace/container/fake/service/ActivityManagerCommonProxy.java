...
import com.prismspace.container.fake.provider.FileProviderHandler;
...
import com.prismspace.container.utils.compat.StartActivityCompat;
...
public class ActivityManagerCommonProxy {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (ComponentUtils.isRequestInstall(intent)) {
                ...
                if (file != null && file.exists()) {
                    try {
                        PackageInfo packageInfo = PrismSpaceCore.getPackageManager().getPackageArchiveInfo(file.getAbsolutePath(), 0);
                        if (packageInfo != null) {
                            String packageName = packageInfo.packageName;
                            String hostPackageName = PrismSpaceCore.getHostPkg();
                            if (packageName.equals(hostPackageName)) {
                                Slog.w(TAG, "Blocked attempt to install PrismSpace host app from inside PrismSpace: " + packageName);
                                
                                return 0;
                            }
                        }
                    } catch (Exception e) {
                        Slog.w(TAG, "Could not verify whether this is a PrismSpace host app: " + e.getMessage());
                    }
                }
                
                if (PrismSpaceCore.get().requestInstallPackage(file, PActivityThread.getUserId())) {
                    return 0;
                }
            ...
            if (dataString != null && dataString.equals("package:" + PActivityThread.getAppPackageName())) {
                intent.setData(Uri.parse("package:" + PrismSpaceCore.getHostPkg()));
            }

            ResolveInfo resolveInfo = PrismSpaceCore.getBPackageManager().resolveActivity(
                    intent,
                    GET_META_DATA,
            ...
            if (resolveInfo == null) {
                ...
                resolveInfo = PrismSpaceCore.getBPackageManager().resolveActivity(
                        intent,
                        GET_META_DATA,
                        StartActivityCompat.getResolvedType(args),
                        PActivityThread.getUserId());
            ...
            PrismSpaceCore.getBActivityManager().startActivityAms(PActivityThread.getUserId(),
    ...
    public static class StartActivities extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            return PrismSpaceCore.getBActivityManager().startActivities(PActivityThread.getUserId(),
    ...
    public static class ActivityResumed extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            PrismSpaceCore.getBActivityManager().onActivityResumed((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class ActivityDestroyed extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            PrismSpaceCore.getBActivityManager().onActivityDestroyed((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class FinishActivity extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            PrismSpaceCore.getBActivityManager().onFinishActivity((IBinder) args[0]);
            return method.invoke(who, args);
        }
    }
    ...
    public static class getCallingPackage extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            return PrismSpaceCore.getBActivityManager().getCallingPackage((IBinder) args[0], PActivityThread.getUserId());
        }
    }
    ...
    public static class getCallingActivity extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            return PrismSpaceCore.getBActivityManager().getCallingActivity((IBinder) args[0], PActivityThread.getUserId());
        }
    }
}
