...
import com.prismspace.container.fake.hook.IInjectHook;
...
public class HCallbackProxy implements IInjectHook, Handler.Callback {
    ...
    private Handler getH() {
        Object currentActivityThread = PrismSpaceCore.mainThread();
    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (PActivityThread.getAppConfig() == null) {
                if (PrismSpaceCore.get().isPackageRestartSuppressed(activityInfo.packageName, stubRecord.mUserId)) {
                    Slog.d(TAG, "Suppressing auto-restart for package: " + activityInfo.packageName + " userId=" + stubRecord.mUserId);
                    return false;
                }
                PrismSpaceCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);

                Intent launchIntentForPackage = PrismSpaceCore.getBPackageManager().getLaunchIntentForPackage(activityInfo.packageName, stubRecord.mUserId);
            ...
            PrismSpaceCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);

            if(PuildCompat.isTiramisu()){
                LaunchActivityItemContext launchActivityItemContext = BRLaunchActivityItem.get(r);
                launchActivityItemContext._set_mIntent(stubRecord.mTarget);
                launchActivityItemContext._set_mInfo(activityInfo);
            } else if (PuildCompat.isS()) {
                Object record = BRActivityThread.get(PrismSpaceCore.mainThread()).getLaunchingActivity(token);
                ...
                clientRecordContext._set_activityInfo(activityInfo);
                clientRecordContext._set_packageInfo(PActivityThread.currentActivityThread().getPackageInfo());

                checkActivityClient();
            } else if (PuildCompat.isPie()) {
                LaunchActivityItemContext launchActivityItemContext = BRLaunchActivityItem.get(r);
                launchActivityItemContext._set_mIntent(stubRecord.mTarget);
                launchActivityItemContext._set_mInfo(activityInfo);
            } else {
    ...
    private boolean handleCreateService(Object data) {
        if (PActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(PActivityThread.getAppPid()))
                ...
                PrismSpaceCore.getBActivityManager().startService(intent, null, false, PActivityThread.getUserId());
                return true;
            }
        }
        return false;
