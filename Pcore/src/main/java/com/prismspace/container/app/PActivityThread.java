...
import com.prismspace.container.app.dispatcher.AppServiceDispatcher;
import com.prismspace.container.core.CrashHandler;
import com.prismspace.container.core.ClonePolicyClient;
...
import com.prismspace.container.core.env.VirtualRuntime;
import com.prismspace.container.core.system.user.PUserHandle;
import com.prismspace.container.entity.AppConfig;
import com.prismspace.container.entity.am.ReceiverData;
...
import com.prismspace.container.utils.SafeContextWrapper;
import com.prismspace.container.utils.GlobalContextWrapper;
import com.prismspace.container.utils.Slog;
import com.prismspace.container.utils.compat.ActivityManagerCompat;
import com.prismspace.container.utils.compat.PuildCompat;
import com.prismspace.container.utils.compat.ContextCompat;
import com.prismspace.container.utils.compat.StrictModeCompat;
import com.prismspace.container.core.system.JarManager;


public class PActivityThread extends IBActivityThread.Stub {
    public static final String TAG = "PActivityThread";

    private static PActivityThread sBActivityThread;
    private AppBindData mBoundApplication;
    private Application mInitialApplication;
    private AppConfig mAppConfig;
    private final List<ProviderInfo> mProviders = new ArrayList<>();
    private final Handler mH = PrismSpaceCore.get().getHandler();
    ...
    public static PActivityThread currentActivityThread() {
        if (sBActivityThread == null) {
            synchronized (PActivityThread.class) {
                if (sBActivityThread == null) {
                    sBActivityThread = new PActivityThread();
                }
            }
        }
    ...
    public static int getBUid() {
        return getAppConfig() == null ? PUserHandle.AID_APP_START : getAppConfig().buid;
    }

    public static int getBAppId() {
        return PUserHandle.getAppId(getBUid());
    }

    public static int getCallingBUid() {
        return getAppConfig() == null ? PrismSpaceCore.getHostUid() : getAppConfig().callingBUid;
    }
    ...
    public Service createService(ServiceInfo serviceInfo, IBinder token) {
        ...
        try {
            Context context = PrismSpaceCore.getContext().createPackageContext(
    ...
    public void bindApplication(final String packageName, final String processName) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            ...
            PrismSpaceCore.get().getHandler().post(() -> {
    ...
    private Object createBindApplicationData(String packageName, String processName) {
        try {
            ...
            PackageInfo packageInfo = PrismSpaceCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, getUserId());
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        PackageInfo packageInfo = PrismSpaceCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, PActivityThread.getUserId());
        ApplicationInfo applicationInfo = packageInfo.applicationInfo;
        ...
        Object boundApplication = BRActivityThread.get(PrismSpaceCore.mainThread()).mBoundApplication();
        ...
        NativeCore.startupInit(Build.VERSION.SDK_INT);
        assert packageContext != null;
        IOCore.get().enableRedirect(packageContext);
        // Ensure child process receives latest published policy via H5.2 service bridge.
        ClonePolicyClient.get().bind(packageContext);
        ...
        try {
            ...
            BRActivityThread.get(PrismSpaceCore.mainThread())._set_mInitialApplication(mInitialApplication);
            ContextCompat.fix((Context) BRActivityThread.get(PrismSpaceCore.mainThread()).getSystemContext());
    ...
    private android.content.pm.ApplicationInfo getApplicationInfo(Object data) {
        try {
            ...
            String packageName = PrismSpaceCore.getAppPackageName();
            if (packageName != null) {
                PackageInfo packageInfo = PrismSpaceCore.getBPackageManager().getPackageInfo(packageName, 0, getUserId());
    ...
    private Context createFallbackContext(String packageName) {
        try {
            Context baseContext = PrismSpaceCore.getContext();
            if (baseContext == null) {
                Slog.e(TAG, "PrismSpaceCore.getContext() is null, cannot create fallback context");
                return null;
            }
    ...
    private void setApplication(Application application) {
        try {
            mInitialApplication = application;
            BRActivityThread.get(PrismSpaceCore.mainThread())._set_mInitialApplication(application);
            ...
            for (ProviderInfo providerInfo : providers) {
                try {
                    if (processName.equals(providerInfo.processName) ||
                        ...
                        installProvider(PrismSpaceCore.mainThread(), context, providerInfo, null);
                    }
    ...
    public static Context createPackageContext(ApplicationInfo info) {
        try {
            return PrismSpaceCore.getContext().createPackageContext(info.packageName,
                    Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static Context createWrappedBaseContext(String packageName) {
        try {
            ...
            return PrismSpaceCore.getContext();
        }
    }
    ...
    public IBinder getActivityThread() {
        return BRActivityThread.get(PrismSpaceCore.mainThread()).getApplicationThread();
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        for (String auth : split) {
            ContentProviderClient contentProviderClient = PrismSpaceCore.getContext()
    ...
    public void finishActivity(final IBinder token) {
        mH.post(() -> {
            Map<IBinder, Object> activities = BRActivityThread.get(PrismSpaceCore.mainThread()).mActivities();
    ...
    public void handleNewIntent(final IBinder token, final Intent intent) {
        mH.post(() -> {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                newIntent = BRReferrerIntent.get()._new(intent, PrismSpaceCore.getHostPkg());
            } else {
                newIntent = intent;
            }
            Object mainThread = PrismSpaceCore.mainThread();
            if (BRActivityThread.get(PrismSpaceCore.mainThread())._check_performNewIntents(null, null) != null) {
    ...
    public void scheduleReceiver(ReceiverData data) throws RemoteException {
        ...
        mH.post(() -> {
            ...
            try {
                ...
                PrismSpaceCore.getBActivityManager().finishBroadcast(data.data);
    ...
    public static Activity getActivityByToken(IBinder token) {
        Map<IBinder, Object> iBinderObjectMap =
                BRActivityThread.get(PrismSpaceCore.mainThread()).mActivities();
    ...
    private void onBeforeCreateApplication(String packageName, String processName, Context context) {
        for (AppLifecycleCallback appLifecycleCallback : PrismSpaceCore.get().getAppLifecycleCallbacks()) {
            appLifecycleCallback.beforeCreateApplication(packageName, processName, context, PActivityThread.getUserId());
        }
    }

    private void onBeforeApplicationOnCreate(String packageName, String processName, Application application) {
        for (AppLifecycleCallback appLifecycleCallback : PrismSpaceCore.get().getAppLifecycleCallbacks()) {
            appLifecycleCallback.beforeApplicationOnCreate(packageName, processName, application, PActivityThread.getUserId());
        }
    }

    private void onAfterApplicationOnCreate(String packageName, String processName, Application application) {
        for (AppLifecycleCallback appLifecycleCallback : PrismSpaceCore.get().getAppLifecycleCallbacks()) {
            appLifecycleCallback.afterApplicationOnCreate(packageName, processName, application, PActivityThread.getUserId());
        }
    }
        ...
        try {
            ...
            try {
                ...
                if (validContext == null) {
                    validContext = PrismSpaceCore.getContext();
                }
            } catch (Exception e) {
