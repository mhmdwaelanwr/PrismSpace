...
public class PActivityManagerService extends IBActivityManagerService.Stub implements ISystemService {
    public static final String TAG = "PActivityManagerService";
    private static final PActivityManagerService sService = new PActivityManagerService();
    private final Map<Integer, UserSpace> mUserSpace = new HashMap<>();
    private final PPackageManagerService mPms = PPackageManagerService.get();
    private final ProadcastManager mBroadcastManager;

    public static PActivityManagerService get() {
        return sService;
    }

    public PActivityManagerService() {
        mBroadcastManager = ProadcastManager.startSystem(this, mPms);
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        ProcessRecord processRecord = PProcessManagerService.get().startProcessLocked(providerInfo.packageName,
                providerInfo.processName,
                PProcessManagerService.get().getUserIdByCallingPid(callingPid),
                -1,
    ...
    public Intent sendBroadcast(Intent intent, String resolvedType, int userId) throws RemoteException {
        List<ResolveInfo> resolves = PPackageManagerService.get().queryBroadcastReceivers(intent, GET_META_DATA, resolvedType, userId);

        for (ResolveInfo resolve : resolves) {
            ProcessRecord processRecord = PProcessManagerService.get().findProcessRecord(resolve.activityInfo.packageName, resolve.activityInfo.processName, userId);
            if (processRecord == null) {
                continue;
            }
            try {
        ...
        shadow.setPackage(PrismSpaceCore.getHostPkg());
    ...
    public void onActivityCreated(int taskId, IBinder token, IBinder activityRecord) throws RemoteException {
        int callingPid = Binder.getCallingPid();
        ProcessRecord process = PProcessManagerService.get().findProcessByPid(callingPid);
        if (process == null) {
            return;
        }
    ...
    public RunningAppProcessInfo getRunningAppProcesses(String callerPackage, int userId) throws RemoteException {
        ...
        List<ProcessRecord> packageProcessAsUser = PProcessManagerService.get().getPackageProcessAsUser(callerPackage, userId);
    ...
    public void scheduleBroadcastReceiver(Intent intent, PendingResultData pendingResultData, int userId) throws RemoteException {
        List<ResolveInfo> resolves = PPackageManagerService.get().queryBroadcastReceivers(intent, GET_META_DATA, null, userId);
    ...
    public void forceStopPackage(String packageName, int userId) {
        UserSpace userSpace = getOrCreateSpaceLocked(userId);
        synchronized (userSpace.mStack) {
            userSpace.mStack.finishAllActivitiesForPackage(packageName, userId);
        }
        synchronized (userSpace.mActiveServices) {
            userSpace.mActiveServices.stopServicesForPackage(packageName, userId);
        }
    }
    ...
    public AppConfig initProcess(String packageName, String processName, int userId) throws RemoteException {
        ProcessRecord processRecord = PProcessManagerService.get().startProcessLocked(packageName, processName, userId, -1, Binder.getCallingPid());
        if (processRecord == null)
            return null;
    ...
    public void restartProcess(String packageName, String processName, int userId) throws RemoteException {
        PProcessManagerService.get().restartAppProcess(packageName, processName, userId);
    }
