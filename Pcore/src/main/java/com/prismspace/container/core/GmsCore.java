...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        PrismSpaceCore blackBoxCore = PrismSpaceCore.get();
        for (String packageName : list) {
            if (blackBoxCore.isInstalled(packageName, userId)) {
                continue;
            }
            try {
                PrismSpaceCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
    ...
    public static boolean isSupportGms() {
        try {
            PrismSpaceCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return PrismSpaceCore.get().isInstalled(GMS_PKG, userId);
    }
}
