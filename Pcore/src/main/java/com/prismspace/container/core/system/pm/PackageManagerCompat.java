...
public class PackageManagerCompat {
    ...
    public static PackageInfo generatePackageInfo(PPackageSettings ps, int flags, PPackageUserState state, int userId) {
        if (ps == null) {
            return null;
        }
        PPackage p = ps.pkg;
    ...
    public static PackageInfo generatePackageInfo(PPackage p, int flags, long firstInstallTime, long lastUpdateTime, PPackageUserState state, int userId) {
        ...
        if ((flags & PackageManager.GET_ACTIVITIES) != 0) {
            ...
            if (N > 0) {
                ...
                for (int i = 0; i < N; i++) {
                    final PPackage.Activity a = p.activities.get(i);
        ...
        if ((flags & PackageManager.GET_RECEIVERS) != 0) {
            ...
            if (N > 0) {
                ...
                final ActivityInfo[] res = new ActivityInfo[N];
                for (int i = 0; i < N; i++) {
                    final PPackage.Activity a = p.receivers.get(i);
                    res[num++] = generateActivityInfo(a, flags, state, userId);
                }
        ...
        if ((flags & PackageManager.GET_SERVICES) != 0) {
            ...
            if (N > 0) {
                ...
                for (int i = 0; i < N; i++) {
                    final PPackage.Service s = p.services.get(i);
        ...
        if ((flags & PackageManager.GET_PROVIDERS) != 0) {
            ...
            if (N > 0) {
                ...
                for (int i = 0; i < N; i++) {
                    final PPackage.Provider pr = p.providers.get(i);
        ...
        try {
            base = PrismSpaceCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
    ...
    public static ActivityInfo generateActivityInfo(PPackage.Activity a, int flags, PPackageUserState state, int userId) {
        ...
        ai.processName = PPackageManagerService.fixProcessName(ai.packageName, ai.processName);
        ai.applicationInfo = generateApplicationInfo(a.owner, flags, state, userId);
        return ai;
    }

    public static ServiceInfo generateServiceInfo(PPackage.Service s, int flags, PPackageUserState state, int userId) {
        ...
        si.processName = PPackageManagerService.fixProcessName(si.packageName, si.processName);
        si.applicationInfo = generateApplicationInfo(s.owner, flags, state, userId);
        return si;
    }

    public static ProviderInfo generateProviderInfo(PPackage.Provider p, int flags, PPackageUserState state, int userId) {
        ...
        pi.processName = PPackageManagerService.fixProcessName(pi.packageName, pi.processName);
    ...
    public static PermissionInfo generatePermissionInfo(
            PPackage.Permission p, int flags) {
    ...
    public static InstrumentationInfo generateInstrumentationInfo(
            PPackage.Instrumentation i, int flags) {
    ...
    public static ApplicationInfo generateApplicationInfo(PPackage p, int flags, PPackageUserState state, int userId) {
        if (!checkUseInstalledOrHidden(flags, state, p.applicationInfo)) {
            return null;
        }
        ApplicationInfo baseApplication;
        try {
            baseApplication = PrismSpaceCore.getPackageManager().getApplicationInfo(PrismSpaceCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = PrismSpaceCore.getPackageManager()
        ...
        ai.processName = PPackageManagerService.fixProcessName(p.packageName, ai.packageName);
    ...
    private static boolean checkUseInstalledOrHidden(int flags, PPackageUserState state,
    ...
    public static Resources getResources(Context context, ApplicationInfo appInfo) {
        PPackageSettings ps = PPackageManagerService.get().getBPackageSetting(appInfo.packageName);
        if (ps != null) {
