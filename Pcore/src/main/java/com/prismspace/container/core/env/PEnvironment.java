package com.prismspace.container.core.env;
...
public class PEnvironment {
    private static final File sVirtualRoot = new File(PrismSpaceCore.getContext().getCacheDir().getParent(), "prismspace");
    private static final File sExternalVirtualRoot = PrismSpaceCore.getContext().getExternalFilesDir("prismspace");
    ...
    public static File getXSharedPreferences(String packageName, String prefFileName) {
       return new File(PEnvironment.getDataDir(packageName, PrismSpaceCore.getUserId()), "shared_prefs/" + prefFileName + ".xml");
    }
}
