...
import com.prismspace.container.core.system.pm.PackageManagerCompat;
import com.prismspace.container.proxy.ProxyActivity;
import com.prismspace.container.proxy.ProxyManifest;
import com.prismspace.container.proxy.record.ProxyActivityRecord;
import com.prismspace.container.utils.ComponentUtils;
...
public class ActivityStack {
    ...
    public ActivityStack() {
        mAms = (ActivityManager) PrismSpaceCore.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    }
    ...
    public int startActivityLocked(int userId, Intent intent, String resolvedType, IBinder resultTo, String resultWho, int requestCode, int flags, Bundle options) {
        ...
        ResolveInfo resolveInfo = PPackageManagerService.get().resolveActivity(intent, GET_ACTIVITIES, resolvedType, userId);
    ...
    private Intent startActivityProcess(int userId, Intent intent, ActivityInfo
        ...
        ProcessRecord targetApp = PProcessManagerService.get().startProcessLocked(info.packageName, info.processName, userId, -1, Binder.getCallingPid());
        if (targetApp == null) {
    ...
    private int startActivityInNewTaskLocked(int userId, Intent intent, ActivityInfo
        ...
        PrismSpaceCore.getContext().startActivity(shadow);
        return 0;
    }
    ...
    private int realStartActivityLocked(IInterface appThread, Intent intent, String resolvedType,
        ...
        try {
            ...
            BRIActivityManager.get(BRActivityManagerNative.get().getDefault()).startActivity(appThread, PrismSpaceCore.getHostPkg(), intent,
    ...
    private Intent getStartStubActivityIntentInner(Intent intent, int vpid,
        ...
        try {
            Resources resources = PackageManagerCompat.getResources(PrismSpaceCore.getContext(), activityInfo.applicationInfo);
            int id;
            ...
            if (windowIsTranslucent) {
                shadow.setComponent(new ComponentName(PrismSpaceCore.getHostPkg(), ProxyManifest.TransparentProxyActivity(vpid)));
            } else {
                shadow.setComponent(new ComponentName(PrismSpaceCore.getHostPkg(), ProxyManifest.getProxyActivity(vpid)));
            }
    ...
    public void hardRemovePackageTasks(String packageName, int userId) {
        synchronized (mTasks) {
            synchronizeTasks();
            Iterator<Map.Entry<Integer, TaskRecord>> taskIterator = mTasks.entrySet().iterator();
            while (taskIterator.hasNext()) {
                TaskRecord task = taskIterator.next().getValue();
                Iterator<ActivityRecord> activityIterator = task.activities.iterator();
                while (activityIterator.hasNext()) {
                    ActivityRecord activity = activityIterator.next();
                    if (activity == null || activity.info == null) {
                        continue;
                    }
                    if (activity.userId != userId || !packageName.equals(activity.info.packageName)) {
                        continue;
                    }
                    activityIterator.remove();
                    mHandler.removeMessages(LAUNCH_TIME_OUT, activity);
                }
        ...
        synchronized (mLaunchingActivities) {
            Iterator<ActivityRecord> launchingIterator = mLaunchingActivities.iterator();
            while (launchingIterator.hasNext()) {
                ActivityRecord activity = launchingIterator.next();
                if (activity == null || activity.info == null) {
                    continue;
                }
                if (activity.userId != userId || !packageName.equals(activity.info.packageName)) {
                ...
                launchingIterator.remove();
                mHandler.removeMessages(LAUNCH_TIME_OUT, activity);
            }
        }
    }

    public void finishAllActivitiesForPackage(String packageName, int userId) {
    ...
    public String getCallingPackage(IBinder token, int userId) {
        synchronized (mTasks) {
            ...
            return PrismSpaceCore.getHostPkg();
        }
    }
    ...
    public ComponentName getCallingActivity(IBinder token, int userId) {
        synchronized (mTasks) {
            ...
            return new ComponentName(PrismSpaceCore.getHostPkg(), ProxyActivity.P0.class.getName());
        }
    }
