...
public class SystemCallProvider extends ContentProvider {
    ...
    private boolean initSystem() {
        try {
            Slog.d(TAG, "Initializing PrismSpace system...");
            PrismSpaceSystem.getSystem().startup();
            Slog.d(TAG, "PrismSpace system initialized successfully");
            return true;
        } catch (Exception e) {
            Slog.e(TAG, "Failed to initialize PrismSpace system", e);
            return false;
        }
    }
        ...
        try {
            ...
            if ("VM".equals(method)) {
                ...
                if (extras != null) {
                    ...
                    if (service != null) {
                        PundleCompat.putBinder(bundle, "_B_|_server_", service);
