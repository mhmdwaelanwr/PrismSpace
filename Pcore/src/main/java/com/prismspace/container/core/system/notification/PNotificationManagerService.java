package com.prismspace.container.core.system.notification;
...
public class PNotificationManagerService extends IBNotificationManagerService.Stub implements ISystemService {
    private final static PNotificationManagerService sService = new PNotificationManagerService();
    ...
    private final NotificationManager mRealNotificationManager =
            (NotificationManager) PrismSpaceCore.getContext().getSystemService(Context.NOTIFICATION_SERVICE);

    public static PNotificationManagerService get() {
        return sService;
    }
    ...
    public void enqueueNotificationWithTag(int id, String tag, Notification notification, int userId) {
        ProcessRecord processByPid = PProcessManagerService.get().findProcessByPid(Binder.getCallingPid());
        if (processByPid == null)
            return;
